...
- **Options panels stayed empty on Classic Era.** On 3.3.5,
  `InterfaceOptions_AddCategory` hides the panel, so the first open fires
  `OnShow`. The modern `Settings` system reparents the canvas without hiding it
  first — `OnShow` never fires, and any panel that fills itself on display stays
  blank. Affected Auto-Roll, Custom Rules, Display and the main panel.

  Two safety nets, because a silent options panel is the worst kind of defect:
  the panel is hidden once to restore the normal cycle, and `OnShow` handlers are
  replayed when the settings window opens, in case the client never calls
  `Show()` on the canvas.

**No behaviour change on Ascension.**
...
- **LootEnh now offers the roll window as a shortcut to the AllEnh hub.** The
  hub knows nothing of our functions — we hand it the action the same way we hand
  it the download URL, through `AllEnh_Register`. An older hub simply ignores the
  field, and no hub at all changes nothing: LootEnh has never depended on it.
